...
class StepCollector:
    def __init__(self, step_traces_dir: Path) -> None:
...
def make_run_paths(task_id: str) -> RunPaths:
    task_root = Path(env_str("BROWSER_USE_TASK_RECORDS_DIR", "task_records"))
    conversations_root = Path(
        env_str("BROWSER_USE_CONVERSATIONS_DIR", "conversations")
    )
...
def build_browser_profile(raw_dir: Path) -> BrowserProfile:
    ...
    return BrowserProfile(
        ...
        dom_highlight_elements=env_bool(
...
def build_artifacts(raw_video_path: Path | None, task_id: str) -> RecordingArtifactsModel:
    ...
    if get_storage_mode() == "uploadthing":
        return upload_video_to_uploadthing(raw_video_path, task_id)
...
async def run_task(task: str, task_id: str | None = None) -> RecordingRunResponse:
    ...
    artifacts = build_artifacts(raw_video_path, effective_task_id)
...
@app.post("/recordings/run", response_model=RecordingRunResponse)
async def run_recording(request: RecordingRunRequest) -> RecordingRunResponse:
    try:
        return await run_task(task=request.task, task_id=request.task_id)
